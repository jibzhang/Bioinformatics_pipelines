...
DMRs=pd.read_table(args.DMRFile, index_col=None, header=None, sep="\t")
outdir = os.path.dirname(args.DMRFile)
...
for methCall in listing:
    ...
    ovlpDF.columns = ['chrom','start','end','chr', 'base', 'base', 'PercMeth', 'freqC', 'freqT']
    #Find percent meth for each CpG
    #ovlpDF.iloc[:,7]=ovlpDF.iloc[:,7].astype(int)
    ...
    avgMeth=ovlpDF.groupby(['chrom', 'start', 'end'])['PercMeth'].mean().reset_index()
