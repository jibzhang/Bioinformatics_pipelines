...
my %opts = (
	f => '/ref_genomes/genome_anno/human/refSeq/hg38/RefSeqAnno.hg38.txt',
	s => 2
);

#my %opts = (
#	f => 'data/hg19/cds_final.pt',
#	s => 2,
#	i => 'test.snv',
#	o => 'test.anno.snv'
#	
...
Options: 
	-i STRING	input poz file name (chrom. start refAllele varAllele end) [required]
...
my $header = <IN>;
chomp $header;
my @titles = qw( mutClass dist2gene transcript gene transStart transEnd strand refCodon varCodon refAa varAa aaPos peptideLen exonPos exonLen);
	...
	if ( $type eq 'utr5' ) {
		if ( $geneType eq 'intron' ) {
			...
			if ( $adjacentExon == 1 ) {
				...
				$exonPoz      = "e$adjacentExon+$adjacentDist";
			}
...
#				my $spExon = $adjacentExon == 1 ? $exonNum - 1 : $exonNum;#introduced error
				my $spExon = $exonNum;
