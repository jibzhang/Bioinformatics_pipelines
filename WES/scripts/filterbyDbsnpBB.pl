...
getopts( 'i:o:b:', \%opts );
print(
	qq/Filter common SNVs in dbsnp database. "bigBedToBed" tool is required.
	In snv file, the fields should be chr, poz, ...
Options: 
	-i STRING	input snv file name [required]
...
while (<IN>) {
	...
	my ( $chr, $poz, $ref, $alt ) = split;
	my $ref_len = length($ref); 
	my $alt_len = length($alt); 
	my $larger_length;
	if ($ref_len > $alt_len) { $larger_length = $ref_len;
	} else {
    	$larger_length = 1;}
	$chr = "chr" . $chr if ( $chr !~ /chr/ );
	my $poz_0 = $poz - 1;
	my $end = $poz_0 + $larger_length;
	print "-chrom=$chr -start=$poz_0 -end=$end";
	my $dbSNP_result =
