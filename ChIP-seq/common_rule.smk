...
#sample list
intake=pd.read_table(config["intake"],encoding="ISO-8859-1")
...
contrastpair = intake["sample_pair"].drop_duplicates()

contrast={}
for i in contrastpair:
    contrast[i]=list(intake[intake["sample_pair"]==i].sample_id.drop_duplicates())

CHIP = [item[0] for item in list(contrast.values())]
input = [item[1] for item in list(contrast.values())]
pair=dict(zip(CHIP, input))
...
for i in set(grouplist):
    comp=list(intake[intake["group"]==i].sample_id.drop_duplicates())
    comparison[i] = [chip for chip in comp if chip in CHIP]
...
squence_type="CHIPseq"
multiqc_name="PRDM1_ChIP"
