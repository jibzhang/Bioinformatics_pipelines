...
rule write_code:
    input:
	    key_smk="PRDM1_CHIP.smk"
    output:
        code=dir_out+"/smk_code/{sample}.sh"
    params:
        cores=4,
        mem="60G",
        log=dir_out+"/smk_log/{sample}_peakcall_%j.log",
        key_file="/coh_labs/jochan/PRDM1_NK/{sample}/CHIPseq/log/{sample}.filter_bed.bmk"
    shell:
        '''
        echo "#!/bin/bash" > {output.code}
        echo "#SBATCH --job-name=peakcall_{wildcards.sample}         # Job name" >> {output.code}
